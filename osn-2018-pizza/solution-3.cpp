...
// against strategy 1: imitate judge's last move if possible
// if not: take any random pizza dengklek like
int get_best_pizza(int last_judge) {
  if(last_judge > 0 && pizza[last_judge] > 0 && dengklek_like[last_judge]) {
    return last_judge;
  }
...
void work() {
  ...
  while(last_judge != 0 || last_dengklek != 0) {
    last_dengklek = get_best_pizza(last_judge);
    pizza[last_dengklek]--;
